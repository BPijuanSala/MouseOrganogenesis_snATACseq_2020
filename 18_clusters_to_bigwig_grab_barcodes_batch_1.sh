...
#SBATCH -e slurm.celltypesBW_batch1.err
...
folderBarcodes=$wd"/08_BigWig_celltypes/barcodes/batch_1/"

files=($(find $folderBarcodes -name "*subset*"))

echo "files" >file_batch1.tmp
...
while (( "$( ls file_batch1.tmp | wc -l | xargs)" < 25)); do
  wait
done
